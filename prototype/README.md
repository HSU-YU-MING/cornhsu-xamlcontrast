...
> ⚠ **這些數字記的是 v0.1.0 當時的行為，不是「正確答案」。**
> 凍結之後在 .NET 側修掉的缺陷，這支腳本一概還帶著 —— 已知至少四個：
> 字色無法解析時錯記進「跳過」而非「無法解析」、Style 配對有一側解不開時
> 連計數都沒有就消失、`MultiTrigger`／`MultiDataTrigger` 整批不受檢、
> `SemiBold` 被當成粗體而誤享 WCAG 大字級豁免（門檻被放寬到 3:1）。
> 所以 `baseline-*.txt` 的「無法解析／跳過」等數字**低報了盲區**。
>
> 它的價值是歷史存證（v0.1.0 長什麼樣），**不可以拿來當正確性的對照組**。
> 現行驗收是 C# 快照回歸：`scripts/verify-baselines.ps1` 比對 `baselines/*.json`。
