...
沿父節點往上找生效背景
  ...
  MultiTrigger / MultiDataTrigger 的組合條件狀態逐一檢（條件為 AND；
    任一條是 IsEnabled=False 即整個狀態屬停用態，WCAG 1.4.3 豁免）
  #FFRRGGBB 的 alpha=255 是完全不透明，不是半透明 —— 那是 Blend／VS
    設計工具的預設輸出格式，當半透明會讓字色整批落進 skipped
  檔案根元素沒寫 Background 時，查該型別的隱含樣式當「地板」
    （僅根元素這一格；完整隱含樣式解析要模擬 WPF 資源查找＋處理 Foreground
     的繼承語意，是另一個量級的工程，見 M6）
```

前十三條是 2026-07-30～31 用四個真實專案一條一條踩出來的（第八條來自 QuillNest
...
字色整批被跳過、另一個讓整個檔案無底可配 —— 同一個盲區在同源樣本上量不出嚴重性，
這是「每條規則來自實查」這條憲法自身的取樣偏誤（第 8.3 節）。
...
App.xaml 的 merge 順序也會影響誰生效。**偵測到同鍵不同值時必須明講**
（列出衝突的鍵），不可默默取其一 —— 2026-08-10 已實作（偵測描述點名衝突鍵）。

**應用程式層作用域（2026-08-10 實作）**：repo 裡有 ≥2 個 `<Application>` 根元素時，
每個 App.xaml 的目錄是一個作用域 —— 各 App 用「自己子樹 ∪ 共用區」的色盤，
共用區（函式庫本體）只用共用區自己的。沒有它的代價（八專案抽驗實證）：
ScreenToGif 的 Translator 子 App、Playnite 的 Desktop/Fullscreen 兩套 ——
鍵名相同值不同，A App 的字被配上 B App 的底，報 1.2:1 的假 fail（實際 10.4 合格）。
單 App repo 完全走原路徑。**元素層 Resources 與 merge 順序仍不解** —— 只切
「不同應用程式」這一刀，那是假警報實際來自的地方。

⚠ 實作時踩過一個「靜默退化」的坑，值得記：作用域目錄經 `GetDirectoryName`
正規化成反斜線，而檔案列舉沿用呼叫端 root 的正斜線寫法 —— `StartsWith` 永遠
不成立，每個檔都被歸到共用區、拿到全域色盤，**功能整個沒作用但無任何錯誤**。
靠探針比對三個作用域的色盤內容才發現（三個一模一樣）。比對路徑一律先
`GetFullPath`；回歸測試改用正斜線 root 覆蓋這條路。

#### 4.2.1 合併式色盤（2026-08-10，外部專案實查後改寫）

原本的「挑 brush 最多的那個檔當色盤」是四個受測專案養出來的啟發法 ——
它們的色票剛好都集中在單一檔案。八個公開專案顯示這在外面普遍失效：

| 專案 | 舊版行為 |
|---|---|
| HandyControl | 色彩分散 8 個檔，brush 與 Color 分屬不同檔且用 `DynamicResource` 跨檔引用 → **完全找不到色盤** |
| MaterialDesignInXamlToolkit | 154 個檔含色彩定義，只用到 1 個（34 鍵） |
| MahApps.Metro | 挑中 `Theme.Template.xaml` —— 建置期樣板，值是 `{{佔位符}}` |
| ILSpy | 挑中反編譯測試的假資料檔當「色盤」 |

現行模型貼近 WPF 的合併字典語意：

1. 色彩字典依路徑提示分成 dark／light／中性三組
2. `darkColors` = 中性 ∪ dark（dark 覆蓋）；`lightColors` = 中性 ∪ light
   —— 只有 dark 提示而無 light 時，中性檔即淺色主題（HandyControl 的形狀：
   預設檔 `Colors.xaml` 不帶 light 字樣，只有深色變體被標記）
3. 所有 brush 字典的每個鍵，各自對兩張 Color 表解析出深淺值
4. 鍵衝突「先出現的贏」，檔案順序固定（字典序），結果可重現
5. 測試／範例／demo／fixture／mock 目錄不當色盤候選（照樣受稽核，只是不能定義顏色）

解析層同時放寬三個假設：屬性順序（`o:Freeze` 排在 `x:Key` 前）、跨行元素、
`DynamicResource` 與 `StaticResource` 同等對待。⚠ 第一項連帶在 QuillNest
（自家受測專案）多抓到 2 個色票、解出 6 組真實配對 —— **同一個缺陷在自家也在
扣覆蓋率，只是四個專案的寫法都沒踩到。**

效果：八個公開專案總覆蓋率 9.6% → 19.2%，無一退步。
...
## 8.3 憲法自身的取樣偏誤（2026-08-10，八個公開專案批次掃描後補記）

「每條規則來自實查」是本專案的憲法，它產出了十三條紮實的規則。
但它有一個內建的盲點：**實查的樣本是四個同一位作者的專案。**

三者的寫法習慣完全同源：色票鍵不帶點、色碼手寫六位數、
根容器直接寫 `Background`。外部專案三樣都不同。後果：

| 缺陷 | 在四個受測專案的代價 | 在外部專案的代價 |
|---|---|---|
| 帶點的色票鍵認不得（定義端 `\w+` vs 使用端 `[\w.]+`） | 0 | ScreenToGif 93 個色票認出 **0 個** |
| `#FFRRGGBB` 被當半透明 | 0 | ScreenToGif 97 個色票有 79 個被整批跳過 |
| 根容器靠隱含樣式給底 | 0（71~86% 直接寫 `Background`） | 整個檔案無底可配 |
| 屬性順序（`o:Freeze` 在 `x:Key` 前） | **不是 0** —— QuillNest 少 2 個色票、6 組配對 | HandyControl 整份漏掉 |

前三項在同源樣本上**量不出嚴重性**；第四項則證明它連自家都在扣分，
只是沒有任何機制會讓它現形 —— 快照回歸只能抓「變了」，抓不到「一開始就錯」。

實測落差：四個受測專案的解析覆蓋率 89~100%，八個公開專案合計 **9.6%**
（修正後 19.2%）。同一個工具、兩個數量級。

**結論寫進憲法**：「每條規則來自實查」要加一句 ——
**實查的樣本必須包含非作者的專案**。這也是 1.0 條件裡「至少一個非作者的專案回報」
那一條的真正理由：它不是禮貌性的社群指標，是唯一能發現這類缺陷的機制。
